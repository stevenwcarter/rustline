...
### T2. Sandbox path resolution returns bare `String`/`PathBuf`, so "resolved" and "allowlisted" are conventions the compiler cannot enforce: `state::resolve_for_allowlist / sanitize_relpath / normalize_abs` (crates/rustline-wasm/src/state.rs:110)
- Lens: newtype (merged with parse-dont-validate — smart-constructor rule)
- Impact: 20 (bug-prevention 5 × blast-radius 4)
- Effort: M (~35 sites, public-API: yes)
- Risk: low
- Blast radius: ~35 sites across 5 files (crates/rustline-wasm/src/{state.rs, perform.rs, capability.rs, allow.rs, cache.rs})
- Proposed type: `pub struct ResolvedPath(String);` (private field; constructible only via `resolve_for_allowlist`/`normalize_abs`, carried by `PathResolveError::SymlinkDenied` too), `pub struct SandboxRelPath(PathBuf);` (only via `sanitize_relpath`), and `pub struct AllowedPath(ResolvedPath);` returned by `AllowSet::check(&self, ResolvedPath) -> Result<AllowedPath, ResolvedPath>` — the allowlist *consumes* a resolved path and *produces* the only token the filesystem effect accepts. Change the four `std::fs`/join effect sites (perform.rs:547 `read_to_string`, perform.rs:601 `fs::write`, and the two `ctx.state_dir().join(rel)` sites at perform.rs:421/452) to take `&AllowedPath`/`&SandboxRelPath`. The module doc's load-bearing "resolution happens strictly before the allowlist check" ordering (state.rs:91-109, perform.rs:513-524) is today enforced only by statement order — `ctx.allowed_paths.allows(raw_guest_string)` compiles identically and silently reopens the symlink-escape gap the resolver exists to close (invariant N1); with the newtypes, "resolve → match → act" is a type-level fact and a future capability that moves the check after the effect fails to compile. `SymlinkDenied` → `observe_denial` routing unchanged. Pure in-memory types — no wire, TOML, or guest-visible change. These are the `ReadPath`/`WritePath` subject types T3's `AllowSet<K>` consumes; execute together if both selected.
- [x] execute   [ ] skip

### T3. CapabilityCtx's four allowlists are interchangeable `AllowSet`s, so a URL can be gated on the path (or write) allowlist with no compile error: `CapabilityCtx.allowed_urls / allowed_paths / allowed_write_paths / allowed_commands` (crates/rustline-wasm/src/capability.rs:64)
- Lens: newtype
- Impact: 20 (bug-prevention 5 × blast-radius 4)
- Effort: L (~42 sites, public-API: yes)
- Risk: low
- Blast radius: ~42 sites across 6 files (crates/rustline-wasm/src/{capability.rs, allow.rs, perform.rs, argv.rs, denials.rs, cache.rs})
- Proposed type: phantom-tagged `pub struct AllowSet<K: CapKind>(Vec<Pattern>, PhantomData<K>);` with zero-sized markers `Url`, `ReadPath`, `WritePath`, `Command` (each `impl CapKind` carrying its `DenialKind`, so `observe_denial`'s second hand-paired argument becomes `K::DENIAL`), plus matching subject newtypes: `Url<'a>(&'a str)`, `ResolvedPath` (from T2), and `CanonicalArgv` — folding in argv.rs: `canonical_argv` returns `CanonicalArgv(String)` (sole constructor `CanonicalArgv::of(program, args)`), required by `AllowSet<Command>::allows` and by the exec cache key, so `allows(program)` (bypassing the whole-argv gate the docs call load-bearing: "a grant for `git status*` does not cover `git push`") and `cache_path(.., program)` (collapsing every invocation of a program onto one cache entry) both stop compiling. `CapabilityCtx::from_config` (capability.rs:113-127) then only typechecks when each field is fed its own `PluginConfig` list — the current four-line positional block is exactly where a copy-paste swap silently converts a read grant into a write grant, the very exploit the `allowed_paths`/`allowed_write_paths` split exists to close. No serde anywhere — `AllowSet` is a compiled in-memory type; `PluginConfig` TOML untouched.
- [x] execute   [ ] skip
